<template>
  <div class="home-head">
    ...
    <div class="stat-row">
      <div class="stat-box">
        <div class="num">{{ stats.doneToday }}/3</div>
    ...
    <div class="quick-row">
      <button class="btn btn-primary" style="flex:1" @click="nav('newCustomer')">+ Customer Baru</button>
      <button class="btn btn-outline" style="flex:1" @click="nav('customers')">Lihat Semua</button>
    </div>
  </div>
  <div class="scroll">
    ...
    <div v-if="plannedCustomers.length" class="visit-list">
      ...
      >
        ...
        <span class="dot" :class="{ done: hasVisitedToday(c.id), pending: !hasVisitedToday(c.id) }"></span>
        <div style="flex:1;">
    ...
    <div v-else class="empty-note">Belum ada customer yang ditugaskan ke Anda.</div>
